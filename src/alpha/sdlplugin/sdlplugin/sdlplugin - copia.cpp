...
int32_t StartTime;
int SendPresence = 1;
const char* APPLICATION_ID = "1200228516554346567";
...
//TUserFunc(void, SetDiscordInfo, Reference ID, Reference State, Reference Details, Reference BigImg, Reference BigTxt, Reference MiniImg, Reference MiniTxt, Reference PartyID, Reference SecretID, Reference SecretJoin, Reference SecretWatch, int PartySize, int PartyMax, int Instance)
TUserFunc(void, SetDiscordInfo, Reference State, Reference Details)
{
	//APPLICATION_ID = pu->refToAstr(CP_UTF8, ID).c_str();
	...
	updateDiscordPresence(); //To Refresh
}
//pu->refToAstr(CP_THREAD_ACP, varName).c_str() //CP_THREAD_ACP For Dirs
...
//Function Prototypes
int PlayVideoTest(HINSTANCE hInstance, PWSTR pCmdLine, int nCmdShow, std::wstring videoRenderer);
LRESULT CALLBACK WindowProc(HWND hwndDirectShow, UINT uMsg, WPARAM wParam, LPARAM lParam);
static void CALLBACK OnGraphEvent(HWND hwndDirectShow, long eventCode, LONG_PTR param1, LONG_PTR param2);
static void OnChar(HWND hwndDirectShow, wchar_t c);
static void OnFileOpen(HWND hwndDirectShow, std::wstring videoRenderer);
static void OnPaint(HWND hwndDirectShow);
static void OnSize(HWND hwndDirectShow);
//static void DestroyGraph(HWND hwndDirectShow);
...
int PlayVideoTest(HINSTANCE hInstance, PWSTR pCmdLine, int nCmdShow, std::wstring videoRenderer)
{
	...
	RECT pRect;
	GetWindowRect(pInfo.info.win.window, &pRect);

	int g_posx = pRect.left;
	...
	hwndDirectShow = CreateWindowEx(0, CUTSCENE_NAME, L"DirectShow Playback", 
        WS_CHILD, 0, 0, g_w, g_h,
		hwndParent, NULL, hInstance, NULL);
	}else{
    hwndDirectShow = CreateWindowEx(0, CUTSCENE_NAME, L"DirectShow Playback", 
        WS_CHILD, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN),
		hwndParent, NULL, hInstance, NULL);
	}
