...
TUserFunc(bool, Init, bool mugen, int32_t h, int32_t w, Reference cap)
{
	if(SDL_Init(SDL_INIT_EVERYTHING) < 0){
		...
		TTF_Init();
		g_scrflag = SDL_SWSURFACE | SDL_WINDOW_BORDERLESS; //SDL_SWSURFACE
		...
		if(mugen){
			g_target =
				SDL_CreateTexture(
					g_renderer, SDL_PIXELFORMAT_ARGB8888, //ARGB8888
...
TUserFunc(intptr_t, AllocSurface, int32_t h, int32_t w)
{
	return
		(intptr_t)SDL_CreateRGBSurface(
			SDL_SWSURFACE, w, h, 32, 0x00FF0000, //32 BPP / Bits / BitsPerPixel
...
int PlayVideoTest(HINSTANCE hInstance, PWSTR pCmdLine, int nCmdShow, std::wstring videoRenderer)
{
    ...
    HWND hwndDirectShow = CreateWindowEx(0, CUTSCENE_NAME, L"DirectShow Playback", 
