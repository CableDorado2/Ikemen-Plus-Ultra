...
--First Check (Tables generated by Sections)
	...
	elseif t_selOptions == nil then --If the Engine not detect Arcade and Survival roster Config in [Options] section from select.def
		t_infoWindowMsg.text = "NO MAXMATCHES CONFIG FOUND IN SELECT.DEF"
		infoScreen = true
		f_exitMenu(true)
	end
	...
	while true do
		...
		for i=1, #t_mainMenu do
			...
			if not infoboxScreen then
				textImgDraw(f_updateTextImg(txt_mainMenuDat, jgFnt, bank, 1, itemSign..itemText, 5, 94 + i * 13 - moveTxt)) --Text Position
			end
		end
		if not infoScreen and not infoboxScreen then
...
function f_desyncWarning()
	t_infoWindowMsg.text = "SET A "..t_aspectPref[aspectRatioPreference].ratio.." RESOLUTION TO AVOID DESYNC"
	infoScreen = true
end

function f_noCharsWarning()
	t_infoWindowMsg.text = "NO CHARACTERS FOUND IN SELECT.DEF"
	infoScreen = true
	f_exitMenu(true)
end

function f_noStagesWarning()
	t_infoWindowMsg.text = "NO STAGES FOUND IN SELECT.DEF"
	infoScreen = true
	f_exitMenu(true)
end
...
function f_exitMenu(titleBGM)
	...
	f_confirmReset()
	exitScreen = false
	if titleBGM then playBGM(bgmTitle) end
	while true do
		...
		for i=1, #t_exitMenu do
			...
			textImgDraw(f_updateTextImg(txt_mainMenuDat, jgFnt, bank, 1, itemSign..t_exitMenu[i].text, 5, 94 + i * 13 - moveTxt))
		end
		if infoScreen then
		...
		drawMiddleMenuSP()
		drawMenuInfo(t_exitMenu[exitMenu])
		textImgDraw(txt_titleFt)
...
function f_towerBoot()
	...
	if #t_selTower ~= 0 then
		...
		t_infoWindowMsg.text = "NO TOWERS FOUND IN SELECT.DEF"
		infoScreen = true
	end
end
...
function f_bonusMenu()
	if #t_bonusChars == 0 then
		t_infoWindowMsg.text = "NO BONUS GAMES FOUND IN SELECT.DEF"
		infoScreen = true
		return
	end
	cmdInput()
	...
	while true do
		if not infoScreen and not sideScreen then
			...
			if timeattackMenu < 1 then
				...
				if #t_timeattackMenu > maxItems then
					cursorPosY = maxItems
				else
			...
			if cursorPosY == maxItems then
				moveTxt = (timeattackMenu - (maxItems + 1)) * 13
			elseif cursorPosY == 0 then
			...
			elseif timeattackMenu - cursorPosY > 0 then
				maxtimeattackMenu = timeattackMenu + maxItems - cursorPosY
			else
				maxtimeattackMenu = maxItems
			end
			if btnPalNo(p1Cmd, true) > 0 or btnPalNo(p2Cmd, true) > 0 then
		...
		if #t_timeattackMenu > maxItems + 1 and maxtimeattackMenu < #t_timeattackMenu then
			animDraw(menuArrowDown)
			animUpdate(menuArrowDown)
		end
...
function f_stageViewer()
	if stageviewer then
		...
		t_infoWindowMsg.text = "NO STAGE VIEWER FOUND IN SELECT.DEF"
		infoScreen = true
	end
end
...
function f_licenseMenu()
	...
	if #t_licenseList == 0 then
		t_infoWindowMsg.text = "NO LICENSES FOUND IN SCREENPACK.LUA"
		infoScreen = true
		return
	end
	cmdInput()
...
function f_replayMenu()
	...
	while true do
		if not infoScreen then
			...
			if cursorPosY == maxItems then
				moveTxt = (replayMenu - (maxItems + 1)) * 13
			elseif cursorPosY == 0 then
			...
			elseif replayMenu - cursorPosY > 0 then
				maxreplayMenu = replayMenu + maxItems - cursorPosY
			else
				maxreplayMenu = maxItems
			end
			if btnPalNo(p1Cmd, true) > 0 or btnPalNo(p2Cmd, true) > 0 then
				sndPlay(sndSys, 100, 1)
		...
		for i=1, #t_replayMenu do
			...
			textImgDraw(f_updateTextImg(txt_mainMenuDat, jgFnt, bank, 1, itemSign..t_replayMenu[i].text, 5, 94 + i * 13 - moveTxt))
		end
		if not infoScreen then
			animSetWindow(cursorBox, 0,97 + cursorPosY * 13, 320,13)
			f_dynamicAlpha(cursorBox, 20,100,5, 255,255,0)
		...
		if #t_replayMenu > maxItems + 1 and maxreplayMenu < #t_replayMenu then
			animDraw(menuArrowDown)
			animUpdate(menuArrowDown)
		end
...
--Display Aspect Ratio Desync Warning
	...
	while true do
		...
		if mainNetplay < 1 then
			...
			if #t_mainNetplay > maxItems then
				cursorPosY = maxItems
			else
		...
		if cursorPosY == maxItems then
			moveTxt = (mainNetplay - (maxItems + 1)) * 13
		elseif cursorPosY == 0 then
		...
		elseif mainNetplay - cursorPosY > 0 then
			maxMainNetplay = mainNetplay + maxItems - cursorPosY
		else
			maxMainNetplay = maxItems
		end
		...
		for i=1, #t_mainNetplay do
			...
			textImgDraw(f_updateTextImg(txt_mainMenuDat, jgFnt, bank, 1, itemSign..t_mainNetplay[i].text, 5, 94 + i * 13 - moveTxt))
		end
		animSetWindow(cursorBox, 0,97 + cursorPosY * 13, 320,13)
		f_dynamicAlpha(cursorBox, 20,100,5, 255,255,0)
		animDraw(f_animVelocity(cursorBox, -1, -1))
		...
		if #t_mainNetplay > maxItems + 1 and maxMainNetplay < #t_mainNetplay then
			animDraw(menuArrowDown)
			animUpdate(menuArrowDown)
		end
		drawMainMenuInputHints(false)
...
function f_mainLobby()
	...
	--Enter Actions
		...
		for i=1, #t_mainLobby do
			...
			textImgDraw(f_updateTextImg(txt_mainMenuDat, jgFnt, bank, 0, itemSign..t_mainLobby[i].text, 159, 94 + i * 13 - moveTxt))
		end
		animSetWindow(cursorBox, 0,97 + cursorPosY * 13, 320,13)
		f_dynamicAlpha(cursorBox, 20,100,5, 255,255,0)
		animDraw(f_animVelocity(cursorBox, -1, -1))
...
function f_intermission() --Secret Fight Intro
	...
	--Draw Approaching Text
		for i = 1, #intermissionTxt do
			textImgDraw(f_updateTextImg(txt_mainMenuDat, jgFnt, 5, 0, intermissionTxt[i], 255, 115 + 12 * (i - 1)))
		end
			...
			if p2numChars > 1 and data.gameMode ~= "alliance" then
				for i=1, #data.t_p2selected do
					if (data.gameMode == "tower" and data.coop) or (data.coop and data.coopenemy == "Single") or
						...
						(t_selChars[data.t_p2selected[i].cel + 1].bonus ~= nil and t_selChars[data.t_p2selected[i].cel + 1].bonus == 1) then
						p2teamMode = 0
						p2numChars = 1
						setTeamMode(2, 0, 1)
...
if validCells() then
	...
	--Tourney Screen Logic
		...
		--Assign Characters to the Match
			...
			data.t_p1selected[#data.t_p1selected + 1] = {
				['cel'] = p1Cell.CharID - 1,
				['name'] = t_selChars[p1Cell.CharID].name,
				['displayname'] = t_selChars[p1Cell.CharID].displayname,
				['path'] = t_selChars[p1Cell.CharID].char,
				['pal'] = p1Cell.pal,
				['up'] = p1Cell.up,
				['rand'] = false
			}
			data.t_p2selected[#data.t_p2selected + 1] = {
				['cel'] = p2Cell.CharID - 1,
				['name'] = t_selChars[p2Cell.CharID].name,
				['displayname'] = t_selChars[p2Cell.CharID].displayname,
				['path'] = t_selChars[p2Cell.CharID].char,
				['pal'] = p2Cell.pal,
				['up'] = p2Cell.up,
				['rand'] = false
			}
...
--[[
	...
	local t_temp = {}
	local update = true
	for i=1, #t_enemyTeam do
		t_enemyBackup[i] = t_enemyTeam[i]
	end
	for i=1, #t_playerTeam do
		t_playerBackup[i] = t_playerTeam[i]
	end
	local function f_resetMembers()
		for i=1, #t_enemyBackup do
			t_enemyTeam[i] = t_enemyBackup[i]
		end
		for i=1, #t_playerBackup do
			t_playerTeam[i] = t_playerBackup[i]
		end
		update = true
	end
	allianceConfirm = false
	...
	while true do
		...
		--Return Logic
			...
			--Skip Exchange	
				if enemySide then
					break
			--Back to Enemy Select to Exchange
				else
					enemySide = true
					f_resetMembers()
	...
	--Exchange Logic
		if not enemySide then
			if update then
				t_temp[2] = t_playerTeam[playerMember]
				update = false
			end
